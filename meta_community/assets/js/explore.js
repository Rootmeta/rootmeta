var app = new Vue({
	...
	mounted: function () {
		...
		Api.exploreDomainList(this, {
			tagId: this.tagId,
			text: this.exploreText
		}, function (rspBody) {
			this.explore = true;
			this.exploreDomainList = rspBody.domainList;
		}, function (code, message) {
			MsgBox.alert(message);
		});
	...
	methods: {
		...
		clickExploredDomain: function (domainId) {
			window.location = '/server/' + Url.encode(domainId);
		},
		clickTag: function (tagIndex, tagId) {
		...
		clickCopyContributeAddress: function () {
			document.querySelector('.contribute .address-text input').select();
